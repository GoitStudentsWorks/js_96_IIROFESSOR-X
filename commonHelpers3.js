import{e as w,o as q}from"./assets/toggle-btn-theme-399e6313.js";import{a as B,i as x}from"./assets/vendor-8cce9181.js";const C="/js_96_IIROFESSOR-X/assets/icons-58ead758.svg";function I(){document.body.insertAdjacentHTML("beforeend",`<button class="scroll-top-btn">
      <svg class="scroll-top-icon" width="15" height="15">

        <use href="${C}#icon-arrow-top"></use>

      </svg>
    </button>
    `);const t=document.querySelector(".scroll-top-btn"),e=document.documentElement.clientHeight;window.addEventListener("scroll",()=>{window.scrollY>e?t.classList.add("show"):window.scrollY<e&&t.classList.remove("show")}),t.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})})}const $=document.querySelector(".exercises-title-span"),l=document.querySelector(".exercises-list"),b=document.querySelector(".exercises-list-pages");b.addEventListener("click",N);l.addEventListener("click",j);const m=document.querySelector(".exercises-form");m.addEventListener("submit",U);m.addEventListener("input",D);const S=document.querySelector(".exercises-button-list"),h=document.querySelector(".form-reset-text-button");h.addEventListener("click",t=>{m.reset(),h.classList.add("reset-button-close")});let p=1,T=8,f="Muscles",d="",v="",E=0;const F=document.querySelectorAll(".exercises-button");[...F].map((t,e)=>{e===0&&t.classList.add("exercises-button-active"),t.addEventListener("click",O)});async function H(t,e,r,i,s,n){let a={};s&&e=="Body parts"&&(e="Body part"),s?a[e.toLowerCase().replace(" ","")]=s:a.filter=e,a.page=r,a.limit=i,n&&(a.keyword=n),document.body.clientWidth>=768&&!s&&(a.limit=12),document.body.clientWidth<=768&&s&&(a.limit=8),document.body.clientWidth>=1440&&s&&(a.limit=9);let o=new URLSearchParams(a).toString();try{const u=await B.get(`https://energyflow.b.goit.study/api/${t}?${o}`);if(E=u.data.totalPages,n&&u.data.results.length==0){A();return}return u.data}catch(u){console.log(u)}}function A(){l.innerHTML=`<div class="no-results-wrapper">
    ...
    opportunity to find more options that suit your needs.
      ...
      rgba(16, 16, 16, 0.7) 100%
          ...
          <p class="exercises-item-text-second" data-type="${o}">${n}</p>
                      ...
                      />
                    </svg></li> `,i=`<li data-id="right"><svg class="page-choice-svg" data-id="right" data-type="${f}" data-card="${d}" xmlns="http://www.w3.org/2000/svg" id="Outline"
                        viewBox="0 0 24 24" width="512" height="512"><path d="M7,24a1,1,0,0,1-.71-.29,1,1,0,0,1,0-1.42l8.17-8.17a3,3,0,0,0,0-4.24L6.29,1.71A1,1,0,0,1,7.71.29l8.17,8.17a5,5,0,0,1,0,7.08L7.71,23.71A1,1,0,0,1,7,24Z" /></svg></li> `,s=0,n="",a=0;if(e<6)return s=1,a=e+1,o(s,a);if(e>5&&t>0&&t<4)return a=5+1,n=o(s+1,a)+i,n;if(t>3&&t+2<e)return s=t-2,a=t+3,r+o(s,a)+i;if(e<=t+2)return e==t+2?s=t-2:e-t==1?s=t-3:s=t-4,r+o(s,e+1);function o(u,y){let g="";for(let c=u;c<y;c++)t==c&&(g+=`<li data-type="${f} data-number="${c}">
          <button class="exercises-pages-button select-pages-ative" data-card="${d}"
           data-number="${c}" data-type="${f}">${c}</button>
        </li>`),t!=c&&(g+=`<li data-type="${f}" data-number="${c}">
          <button class="exercises-pages-button" data-card="${d}"
           data-number="${c}" data-type="${f}">${c}</button>
        </li>`);return g}}function j(t){if(t.target.dataset.type){l.innerHTML='<div class="loder-container-card" ><div class="loader"></div></div>';let e=document.querySelector(".exercises-button-active").dataset.name;e==="Body parts"&&(e="Body part"),d=t.target.dataset.type,p=1,m.classList.remove("form-is-hiden"),b.innerHTML="",S.classList.add("form-is-open"),$.innerHTML=`<span class="exer-title-slash">/</span> ${d.slice(0,1).toUpperCase()}${d.slice(1)}`,L(e,d)}t.target.dataset.id&&q(t.target.dataset.id)}async function L(t,e,r=""){try{const i=await H("exercises",t,p,T,e,r);i&&k(i.results,i.totalPages)}catch(i){console.log(i)}}function U(t){if(t.preventDefault(),v=t.target.elements.usertext.value.trim(),v.length!==0){const e=t.target.dataset.filter;t.target.dataset.status="ok",m.reset(),L(e,d,v)}}function D(t){t.target.value.length>2?h.classList.remove("reset-button-close"):h.classList.add("reset-button-close")}I();document.querySelector(".footer-form").addEventListener("submit",function(t){t.preventDefault();const r=this.elements.email.value.trim();if(!/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/.test(r)){x.success("Field must be filled in");return}fetch("https://energyflow.b.goit.study/api/subscription",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r})}).then(s=>{s.status>=200&&s.status<300&&(x.success({position:"topRight",message:"Subscription successful!"}),this.reset())}).catch(s=>{console.error(s),x.error("Sorry! Failed to subscribe."),this.reset()})});
