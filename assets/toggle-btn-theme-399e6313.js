import{a as M,i as p}from"./vendor-8cce9181.js";(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function o(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=o(e);fetch(e.href,i)}})();const f={mobileMenu:document.querySelector("[data-mobile-menu]"),mobileMenuBtnClose:document.querySelector("[data-mobile-menu-close]"),mobileMenuBtnOpen:document.querySelector("[data-modal-open]")};f.mobileMenu.classList.contains("is-hidden")&&f.mobileMenuBtnOpen.addEventListener("click",O);function O(){f.mobileMenu.classList.remove("is-hidden"),f.mobileMenuBtnClose.addEventListener("click",R),f.mobileMenuBtnOpen.removeEventListener("click",O)}function R(){f.mobileMenu.classList.add("is-hidden"),f.mobileMenuBtnClose.removeEventListener("click",R),f.mobileMenuBtnOpen.addEventListener("click",O)}const k="/js_96_IIROFESSOR-X/assets/symbol-defs-dfb5c4f7.svg";let C,c,v=1;const d={favoritesList:document.querySelector(".favorites-list"),gymInfo:document.querySelector(".gym-info"),modalBtnFavorites:document.querySelector(".modal-button-favorites")};window.addEventListener("load",H);const S=t=>{if(document.body.clientWidth<=768&&c.length>0&&(t=V()),d.favoritesList){const n=t.reduce((o,{_id:a,name:e,burnedCalories:i,time:r,bodyPart:s,target:g})=>o+`<li class="exer-card-item">
    ...
    <p class="card-target-text">
      ...
      <span class="card-burned-calories-span">${g}</span>
          ...
          <button class="exercises-pages-button" 
           data-number="${u}" >${u}</button>
        </li>`);return m}}let I="",A,E;function _(t){I=t,document.querySelector(".modal-button-rating").addEventListener("click",n=>{L(),K()})}function K(){const t=`
  ...
  `,n=document.querySelector(".container-for-modal");n.innerHTML=t;const o=document.querySelector(".rating-modal-backdrop");document.querySelectorAll(".rating-modal-backdrop .rating-modal-close-button").forEach(e=>{e.addEventListener("click",()=>{b()})}),document.addEventListener("keydown",e=>{e.key==="Escape"&&b()}),o.addEventListener("click",e=>{e.target===o&&b()}),U(),X()}function b(){const t=document.querySelector(".rating-modal-backdrop");t&&(t.remove(),B(I))}function U(){const t=document.querySelector(".rating-container.rating-set.rating-modal-container");t&&(n(t),o(),a(t));function n(e){A=e.querySelector(".rating-active.rating-modal-active"),E=e.querySelector(".rating-value.rating-modal-value")}function o(e=E.innerHTML){const i=e/.05;A.style.width=`${i}%`}function a(e){const i=e.querySelectorAll(".rating-item.rating-modal-item");for(let r=0;r<i.length;r++){const s=i[r];s.addEventListener("mouseenter",function(g){n(e),o(s.value)}),s.addEventListener("mouseleave",function(g){o()}),s.addEventListener("click",function(g){n(e);const l=(g.clientX-s.getBoundingClientRect().left)/s.clientWidth,m=Math.min(5,r+l);E.innerHTML=m.toFixed(1),o(m)})}e.addEventListener("mouseleave",function(r){e.classList.remove("interacting")})}}function X(){document.querySelector(".button-modal-rating-send").addEventListener("click",async n=>{n.preventDefault();const o=document.getElementById("email-modal"),a=document.getElementById("user-comment"),e=parseFloat(document.querySelector(".rating-value").innerText);if(o.checkValidity()&&a.checkValidity()&&e>=0){let i={rate:e,email:o.value,review:a.value};try{await M.patch(`https://energyflow.b.goit.study/api/exercises/${I}/rating/`,i),b()}catch(r){console.error(r.response.data),p.error({title:"Error",message:"Failed to submit the rating. Please try again later."})}}else p.warning({title:"Warning",message:"Please fill in all the required fields."})})}const w=document.querySelector(".container-for-modal"),B=t=>{try{M({method:"get",url:`https://energyflow.b.goit.study/api/exercises/${t}`}).then(o=>{Z(o.data),z(o.data.rating),G(o.data)}).catch(o=>{console.log(o)}).finally(()=>{w.removeEventListener("click",B),document.addEventListener("click",N),document.addEventListener("keydown",F)})}catch(n){console.error(n)}};function Z(t={}){const n=`
        <div class="overlay">
        ...
        </div>`;w.innerHTML=n,_(t._id)}function z(t){const n=t/5*100,o=`${Math.round(n/10)*10}%`;document.querySelector(".rating .stars-inner").style.width=o}function N(t){(t.target.classList.contains("modal-close-button")||t.target.classList.contains("overlay"))&&L()}function F(t){t.key==="Escape"&&L()}function L(){w.innerHTML="",document.removeEventListener("keydown",F),document.removeEventListener("click",N)}const q=()=>{let t=JSON.parse(localStorage.getItem("favorites"));S(t)};function G(t){const n=localStorage.getItem("favorites")||"[]",o=t;let a;try{a=JSON.parse(n)}catch(l){console.log(l),a=[]}Array.isArray(a)||(a=[]);const e=w.querySelector(".modal-button-favorites"),i=e.querySelector(".btn-text"),r=()=>{const l=a.filter(m=>m._id!==o._id);localStorage.setItem("favorites",JSON.stringify(l)),i.textContent="Add to favorites",p.error({message:"Exercise has been removed from Favourites"}),q(),e.removeEventListener("click",r),e.addEventListener("click",s)},s=()=>{let l=!1;l||(a.find(u=>u._id===o._id)?(localStorage.setItem("favorites",JSON.stringify(a)),i.textContent="Remove from",p.success({title:"Success",message:"Exercise has been added to Favourites"}),q(),e.removeEventListener("click",s),e.addEventListener("click",r)):(a.push(o),localStorage.setItem("favorites",JSON.stringify(a)),i.textContent="Remove from",p.success({title:"Success",message:"Exercise has been added to Favourites"}),q(),l=!0,e.removeEventListener("click",s),e.addEventListener("click",r)))};e.removeEventListener("click",r),e.removeEventListener("click",s),a.find(l=>l._id===o._id)?(e.addEventListener("click",r),i.textContent="Remove from"):(e.addEventListener("click",s),i.textContent="Add to favorites")}const Y=document.querySelector(".quote-text"),P=document.querySelector(".quote-author"),tt=async()=>{try{return(await M.get("https://energyflow.b.goit.study/api/quote")).data}catch(t){console.log(t.message)}},et=(t,n)=>{const o={quote:t,author:n,date:new Date().toDateString()};localStorage.setItem("quote-data",JSON.stringify(o))},nt=()=>{const t=localStorage.getItem("quote-data");if(t){const{date:n,quote:o,author:a}=JSON.parse(t);if(n===new Date().toDateString()){j(o,a);return}}at()},j=(t,n)=>{Y.textContent=t,P.textContent=n},at=async()=>{const t=await tt();j(t.quote,t.author),et(t.quote,t.author)};nt();const D=document.getElementById("toggle-btn-theme"),h=document.body;function ot(){D.checked?(h.classList.remove("light-theme"),h.classList.add("dark-theme")):(h.classList.remove("dark-theme"),h.classList.add("light-theme"))}function it(){window.innerWidth<=767&&(document.querySelectorAll(".header-social-list").forEach(a=>{a.classList.remove("visible")}),document.querySelector(".toggle-btn-theme").classList.add("visible"),document.querySelectorAll(".header-social-item").forEach(a=>{a.classList.contains("toggle-container")||a.classList.add("hidden")}))}window.addEventListener("DOMContentLoaded",it);D.addEventListener("change",ot);export{k as e,B as o};
//# sourceMappingURL=toggle-btn-theme-399e6313.js.map
